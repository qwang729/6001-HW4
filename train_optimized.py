#!/usr/bin/env python3
"""
Text Sentiment Classification using Custom LSTM Model - Optimized Version
- No external packages beyond torch, numpy, pandas
- Custom Word2Vec implementation
- Self-training with unlabeled data
- Optimized hyperparameters for better accuracy and lower loss
"""
...
class SimpleVocabEmbedding:
    ...
    def _build_vocab(self, sentences):
        ...
        # Filter by min_count (lower min_count to capture more words)
        ...
        # Initialize embeddings with Xavier-like initialization
        for word in vocab:
...
# ============== Improved LSTM Model with Attention ==============
class LSTM_Net(nn.Module):
    def __init__(self, embedding, embedding_dim, hidden_dim, num_layers, dropout=0.5, fix_embedding=True, bidirectional=False):
        ...
        # Bidirectional LSTM with optimized parameters
        self.lstm = nn.LSTM(
            embedding_dim, 
            hidden_dim, 
        ...
        # Enhanced classifier with multiple layers and better activation
...
# ============== Training Function ==============
def training(batch_size, n_epoch, lr, model_dir, train, valid, model, device, use_amp=False):
...
def main():
    ...
    sen_len = 50  # Increased sequence length for better context
    fix_embedding = False  # Allow embedding to be fine-tuned
    batch_size = 128  # Smaller batch size for better gradient updates
    epoch = 15  # More epochs for better convergence
    ...
    w2v_model = SimpleVocabEmbedding(
        all_sentences, 
        vector_size=300,  # Larger embedding dimension
        min_count=1  # Include more words
    )
    ...
    # Multiple rounds of self-training with unlabeled data
    print("\n=== Self-Training with Unlabeled Data ===")
    ...
    # Multiple iterations of self-training
    for iteration in range(3):
        print(f"\n--- Self-training iteration {iteration + 1} ---")
        
        # Generate pseudo-labels with decreasing threshold
